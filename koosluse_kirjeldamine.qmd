...
![Liigirikkus (S) ei arvesta koosluse liikide ohtrussuhteid. Nii vasakulolev kahe liigiga (must ja valge) kooslus kui paremal olev kooslus on sama liigirikkusega, aga funktsionaalselt ilmselt küllaltki erinevad.](media/liigirikkus.PNG){width="100%"}
...
-   **Shannoni indeks** (**H'**; Shannon index, Shannon's diversity index, Shannon-Wiener index) - on tundlik nii liigirikkuse kui liikide ohtruse ühtluse suhtes.
    ...
    -   **ln** on naturaallogaritm *(miinusmärk on sellepärast, et ln suhtelisest ohtrusest, mis jääb vahemikku \[0;1\] on negatiivne)*

::: callout-note
*Shannoni indeks pärineb tegelikult informatsiooniteooriast, kus seda tuntakse Shannoni entroopia nime all - entroopia termodünaamika mõistes aga teatavasti peegeldab süsteemis valitsevat juhuslikkust, segadust kaost? Mitmekesisuse mõõdikuna on Shannoni indeks aga kõrgem just näiteks kenasti välja kujunenud, hilissuktsessioonilises ja liigirikkas koosluses (võrreldes näiteks muruga). Iva on selles, et matemaatilises mõttes on kena, mitmekesine, ja liikide arvu ruuduga (või enam) võrdelises seoses kasvavate (väljakujunenud) interaktsioonide hulgaga kooslus väga raskesti defineeritav - mustrit (näiteks selle kohta, mis liik isendeid juhuslikult valides järgmisena tuleb) on väga raske leida.*

![Kommentaar Marko Mägi postitusele (22.02.2026, Facebook) Tartu linnalooduse (linnavalitsuse plaan rohealasid "korrastada") kohta](media/aaviksoo_entroopia.png){width="100%"}

Miks on ülaltoodud kommentaar ühtlasi nii õige kui ka ekslik?
:::
...
![](media/shannon_vordne.PNG){width="100%"}

Kõik väga ilus.

Kui aga ohtrused pole võrdsed

![](media/shannon_ebavordne.PNG){width="100%"}
...
**Hilli arvud** (^**q**^**D**;Hill numbers, effective number of species) on mitmekesisusemõõdikute perekond, mille abil on võimalik mitmekesisust väljendada **intuitiivselt** (ja matemaatiliselt korrektsemalt - nad on aditiivsed (**doubling property**) ehk algset kooslust kahekordistades on ka indeksi väärtus kaks korda suurem). Hilli arvude omapäraks on **parameeter q**, mis väljendab dominantsetele liikidele antavat kaalu mitmekesisuse arvutamisel.
...
[![Hilli arve kasutades on võimalik matemaatiliselt ennustada, mis oleks mitmekesisus "täieliku" proovivõtupingutuse puhul - ehk siis kui me koosluse kõik liigid üles leiaksime](media/iNEXT.PNG){width="100%"}](https://doi.org/10.1890/13-0133.1)
...
Ühtluse mõõdikuid on mitmeid erinevaid, ühe näitena saab ühtlust tuletada Hilli arvudest
...
E varieerub 0 .. 1, ja = 1 juhul kui kõikide liikide ohtrused on võrdsed.
:::
